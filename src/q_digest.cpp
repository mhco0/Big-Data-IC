...
void test_update_from_scratch(){
	...
	cout << "Querys[5] : " << endl;
	for(int i=0;i<5;i++){
		cout << "query(" << i << "): " << qdst.query(i) << endl;
		int real_rank = rank_from_stream(stream,i);
		cout << "Range to approximated rank -> ("<< real_rank << " <= rank(x) <= " << (real_rank*1.0 + 0.25*qdst.weight_total()) << ")" << endl; 
	}
...
void test_q_digest(double epsilon,int vector_size,int number_of_updates,int number_of_query,bool shuffle,bool print_tree_on_query){
	...
	for(auto &command:commands){
		if(command.first == -1){
			...
			cout << "Query(" << command.second << "): " << qdst.query(command.second) << endl;
			int real_rank = rank_from_stream(stream,command.second);
			cout << "Range to approximated rank -> ("<< real_rank << " <= rank(x) <= " << (real_rank*1.0 + epsilon*qdst.weight_total()) << ")" << endl; 
			cout << "----------------------------------------------------------------" << endl;
			cout << endl;
		}else{
