...
namespace qsbd {
	...
	count_sketch::count_sketch(int fixd, int fixt){
		...
		for(int i = 0; i < this->d; i++){
			this->hash_functions.emplace_back(2 * this->t);
		}
	}
	...
	count_sketch::count_sketch(int fixd, int fixt, const std::vector<std::vector<int>>& hashs_consts){
		...
		this->hash_functions.reserve(this->d);
		for(int i = 0; i < this->d; i++){
			this->hash_functions.emplace_back(2 * this->t, hashs_consts[i]);
		}
	}
	...
	std::vector<two_wise_family> count_sketch::get_hash_functions() const {
